...
pygame.display.set_caption('Cel mai scurt drum intre poligoane')
...
def help_screen():
    ...
    while helping:
        ...
        draw_text("Cel mai scurt drum intre poligoane", black, 30, startxi + 90, startyi + 10)
        draw_text("Q - Iesire!", black, 25, startxi + 10, startyi + 100)
        draw_text("H - Ajutor!", black, 25, startxi + 10, startyi + 150)
        draw_text("D - Deseneaza poligoane", black, 25, startxi + 10, startyi + 200)
        draw_text("C - Sterge poligoane", black, 25, startxi + 10, startyi + 250)
        draw_text("S - Selecteaza start si finish", black, 25, startxi + 10, startyi + 300)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                ...
                if event.type == pygame.MOUSEBUTTONUP or any(pygame.mouse.get_pressed()):
                    if pygame.mouse.get_pressed()[LEFT - 1]:
                        point = Point(pos[0], pos[1])
                        is_outside = True
                        for polygon in sim.polygons:
                            list = []
                            for punct in polygon:
                                list.append((punct.x, punct.y))
                            result = []
                            for i in list:
                                if i not in result:
                                    result.append(i)
                            auxiliary_polygon = Polygon(result)

                            print(auxiliary_polygon)
                            print("________________ Left ________________")
                            print(point)
                            if auxiliary_polygon.contains(point):
                                is_outside = False
                                messagebox.showinfo('Information', 'Selecteaza un punct din exterior!')
                        if is_outside:
                        ...
                        for polygon in sim.polygons:
                            ...
                            auxiliary_polygon = Polygon(result)
                            print(auxiliary_polygon)
                            print("________________ Right ________________")
                            print(point)
                            if auxiliary_polygon.contains(point):
                                is_outside = False
                                messagebox.showinfo('Information', 'Selecteaza un punct din exterior!')
